...
public final class SendspinClient {
    ...
    private var currentStreamFormat: AudioFormatSpec? // Track active stream to deduplicate
    private var lastStreamStartTime: Date? // Track when stream started to prevent rapid stop
    ...
    public init(
        ...
        if roles.contains(.playerV1) && playerConfig == nil {
            print("[SendspinKit] WARNING: Player role requires playerConfig but none provided")
        }
    }
    ...
    public func connect(to url: URL) async throws {
        ...
        await transport.setDisconnectionHandler { [weak self] in
            Task { @MainActor in
                ...
                print("[SendspinClient] WebSocket disconnected, updating connection state")

                // Update state to disconnected
                self.connectionState = .disconnected

                // Emit error event to notify app layer
                self.eventsContinuation.yield(.error("Connection lost - WebSocket closed"))
            }
        }
    ...
    private func handleStreamEnd(_: StreamEndMessage) async {
        audioPlayer?.stop()
        playerState = .synchronized
        isAutoStarting = false  // Reset so next stream can auto-start
        currentStreamFormat = nil  // Clear so next stream/start is processed
        lastStreamStartTime = nil
    ...
    private func handleStreamStart(_ message: StreamStartMessage) async {
        ...
        // Check if was playing to avoid duplicate events
        let wasPlaying = audioPlayer.isPlaying
        // print("[SendspinKit] wasPlaying: \(wasPlaying), will emit streamStarted: \(!wasPlaying)")

        do {
            // print("[SendspinKit] 🚀 Starting audio player with format: \(format.codec) \(format.sampleRate)Hz")
            try audioPlayer.start(format: format, codecHeader: codecHeader)
            playerState = .synchronized
            isAutoStarting = false  // Successfully started, reset flag
            currentStreamFormat = format  // Track current stream
            lastStreamStartTime = Date()  // Track start time for debugging

            if !wasPlaying {
                // print("[SendspinKit] ✅ Emitting streamStarted event")
                eventsContinuation.yield(.streamStarted(format))
            } else {
                // print("[SendspinKit] ⚠️ Skipping streamStarted event (was already playing)")
            }
            try? await sendClientState()
    ...
    /// - Parameter rate: Playback rate from 0.25 to 4.0 (1.0 = normal speed)
    @MainActor
    public func setPlaybackRate(_ rate: Float) {
        print("[SendspinKit.Client] Setting playback rate to \(rate), audioPlayer exists: \(audioPlayer != nil)")
    ...
    /// Check if transport is actually connected (for health monitoring)
    @MainActor
    public func isTransportConnected() async -> Bool {
        return await transport?.isConnected ?? false
    }
}
