...
public actor WebSocketTransport: NSObject, URLSessionWebSocketDelegate {
    ...
    /// Callback invoked when WebSocket disconnects
    private var onDisconnected: (@Sendable () -> Void)?

    /// Set the disconnection handler (actor-isolated)
    ...
    private func listen() {
        webSocket?.receive { [weak self] result in
            ...
            case .failure(let error):
                ...
                // Notify disconnection handler for error-based disconnections
                Task { @MainActor in
                    await self.handleDisconnection(closeCode: .abnormalClosure)
                }
            }
        }
    }
    ...
    nonisolated public func urlSession(_ session: URLSession, webSocketTask: URLSessionWebSocketTask, didCloseWith closeCode: URLSessionWebSocketTask.CloseCode, reason: Data?) {
        ...
        Task { @MainActor in
            await self.handleDisconnection(closeCode: closeCode)
        }
    }

    private func handleDisconnection(closeCode: URLSessionWebSocketTask.CloseCode) {
        print("[WebSocketTransport] Handling disconnection, notifying client...")
        onDisconnected?()
    }
}
