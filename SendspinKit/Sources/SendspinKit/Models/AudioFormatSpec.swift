...
public struct AudioFormatSpec: Codable, Sendable, Hashable {
    ...
    public init(codec: AudioCodec, channels: Int, sampleRate: Int, bitDepth: Int) {
        assert(channels > 0 && channels <= 32, "Channels must be between 1 and 32")
        assert(sampleRate > 0 && sampleRate <= 384_000, "Sample rate must be between 1 and 384000 Hz")
        assert(bitDepth == 16 || bitDepth == 24 || bitDepth == 32, "Bit depth must be 16, 24, or 32")
